...
class AttendanceSheet(models.Model):
    ...
    tot_worked_hour = fields.Float(compute="_compute_sheet_total",
                                   ...
                                   store=True)
    worked_days = fields.Float(compute="_compute_sheet_total",
                                   string="Total Days", readonly=True,
                                   store=True)
    ...
    def _compute_sheet_total(self):
        ...
        for sheet in self:
            ...
            for line in overtime_lines:
                if line.overtime >= 5 and line.ac_sign_out >= 14.5:
                    no_days += 1
                    total += ((line.overtime - 4) / 2) * 0.5
            sheet.contract_id.no_late_days = no_days
            sheet.contract_id.total_late_bonus = total

            sheet.tot_overtime = sum([l.overtime for l in overtime_lines])
            sheet.no_overtime = len(overtime_lines)
            # Compute Total hours
            attendance_lines = sheet.line_ids.filtered(lambda l: l.worked_hours > 0)
            sheet.tot_worked_hour = sum([l.worked_hours for l in overtime_lines])
            sheet.worked_days = len(attendance_lines)
            ...
            sheet.contract_id.ovr_time = sheet.tot_overtime
            sheet.contract_id.late_time = sheet.tot_late
            sheet.contract_id.absence_days = sheet.no_absence
            sheet.contract_id.number_of_hours = sheet.tot_worked_hour
            sheet.contract_id.number_of_days = sheet.worked_days

            off_lines = sheet.line_ids.filtered(lambda l: l.status in ('weekend', 'ph'))
            sheet.contract_id.number_off_days = len(off_lines)
